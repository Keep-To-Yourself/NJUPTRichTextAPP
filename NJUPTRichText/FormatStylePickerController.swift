...
// BIUS格式选择器控制器
class FormatStylePickerController: UIViewController {
    // 关闭回调
    var onDismiss: (() -> Void)?
    
    // SwiftUI托管控制器
    private var hostingController: UIHostingController<FormatStylePicker>?
    ...
    func showPicker(above anchorView: UIView, attributedText: Binding<NSAttributedString>, currentAttributes: Binding<RichTextFormatting.TextAttributes>, textView: UITextView) {
        ...
        // 创建FormatStylePicker SwiftUI视图
        let formatPicker = FormatStylePicker(
        ...
        // 创建托管控制器
        let hostingController = UIHostingController(rootView: formatPicker)
